...
// call the stored procedure
  ...
  /**
   * Permet d'obtenir toutes les colonnes de la table
   * @return hash [] =>  Field | Type  | Null | Key | Default | Extra
   */
  public function getAllColumns($field=NULL) {
    $requete = "SHOW COLUMNS FROM  " . $this->table;
    if ($field !== NULL) {
      $requete .= " WHERE Field='$field'";
      $return = $this->fetchOne($requete);
    }else{
  ...
  public function isIdAutoIncr() {
    $res = $this->getAllColumns($this->champId);
  ...
  public function insert($columns) {
    $isIdAutoIncr = $this->isIdAutoIncr();
    if ($this->insertHook($columns) !== FALSE) {
      $sql = 'INSERT ' . $this->table . ' (';
      $sep = '';
      foreach ($columns as $key => $value) {
        if ($key !== $this->champId or ($isIdAutoIncr === FALSE && $key === $this->champId)) {
          $sql.="$sep $key";
          $sep = ', ';
        }
      }
      ...
      if ($return !== FALSE) {
        if($isIdAutoIncr === TRUE){
          ...
          if(isset($columns[$this->champId])){
            $return = $columns[$this->champId] ;
          }  else {
            $return = NULL;  
          }
        }
  ...
  public function update($id, $columns) {
    ...
    if ($ligne !== FALSE) {
      if ($this->updateHook($id, $columns, $ligne) !== FALSE) {
        ...
        foreach ($columns as $key => $value) {
          if ($key !== $this->champId or ($isIdAutoIncr === FALSE && $key === $this->champId)) {
            $this->bindParam("update_$key", $value);
          }
        }
        $return = $this->execute();
      }
